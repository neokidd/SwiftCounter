...
//  SwiftCounter
//
//  Created by Yanqiang (Oliver) Luan on 7/13/14.
//  Copyright (c) 2014 Yanqiang (Oliver) Luan. All rights reserved.
//
...
class CounterViewController : UIViewController {
    ...
    var timeButtons: [UIButton]? //time buttons
    let timeButtonInfos = [("5 sec", 5), ("1 min", 60), ("3 min", 180), ("5 min", 300)]
    var someInfos: (String, UIButton)? // to be deleted.
    ...
    willSet(newSeconds) {
        ...
        if newSeconds <= 0 {
            ...
            self.startStopButton!.removeLighting()
            self.startStopButton!.setTitle("启动/停止", forState: UIControlState.Normal)
        } else {
            self.startStopButton!.alpha = 1.0
            self.startStopButton!.enabled = true
            
            createAndFireLocalNotificationAfterSeconds(newSeconds)
        }
...
//        let manager = AFHTTPRequestOperationManager()
    ...
    func setupTimeButtons() {
        ...
        for (index, (title, value)) in enumerate(timeButtonInfos) {
    ...
    func timeButtonTapped(sender: UIButton) {
        let (title, value) = timeButtonInfos[sender.tag]
    ...
    func createAndFireLocalNotificationAfterSeconds(seconds: Int) {
        ...
        let timeIntervalSinceNow = (seconds as NSNumber).doubleValue
